...
// Backgrounds ship in ../resources/backgrounds. Slide tile graphs are sample
// assets that live in the git-ignored fixtureDir; demo tests that need them are
// skipped when it is absent so `go test ./...` stays green in CI.
...
func fixturesAvailable() bool {
	for _, f := range []string{"tile-1.png"} {
...
func requireFixtures(t *testing.T) {
	...
	if !fixturesAvailable() {
		t.Skipf("optional sample assets missing in %s (slide tile graphs)", fixtureDir)
	}
}
